import {
  ...
  ShaderMaterial,
  BackSide,
  Color, AdditiveBlending, Vector2, GLSL3
} from "three"
...
export default class SelectionVoxel {
  ...
  setMaterial() {
    ...
    this.materials.side.blending = AdditiveBlending
    ...
    this.materials.side.uniforms.uResolution = {value: new Vector2(.24, 1.0)}
  ...
  setPosition(x: number, y: number, z: number) {
    this.mesh.position.x = x + this.voxel.width / 2
    this.mesh.position.y = y + (this.voxel.height / 2)
    this.mesh.position.z = z + (this.voxel.size / 2)
  }
  ...
  setDebug() {
    ...
    let side = this.debugFolder.addFolder('Sides')
    let bottom = this.debugFolder.addFolder('Bottom')
    let tmp = {
      sideColor: '#ffee9c',
      bottomColor: '#ffee9c'
    }
